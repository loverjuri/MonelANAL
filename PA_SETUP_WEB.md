...
BOT_USERNAME — имя бота для кнопки «Войти через Telegram» (можно с `@`, приложение его уберёт). Если не задано, виджет не показывается.

### Ошибка `Bot domain invalid`

Это не ошибка Flask. Telegram Login Widget разрешает авторизацию только с домена,
который привязан к боту. В `@BotFather`:

1. Выполните `/setdomain`.
2. Выберите нужного бота.
3. Укажите только домен, без `https://`, пути и завершающего `/`:
   `monocore.pythonanywhere.com`
4. Перезапустите Web app на PythonAnywhere и откройте страницу заново.

Для локальног�� адреса `127.0.0.1` или `localhost` Login Widget обычно не используют:
для локальной проверки включите `DEV_MODE=1` и войдите автоматически, либо используйте
логин/пароль. Для production оставьте `DEV_MODE=0`.
